...
HOW TO RUN:
    ...
    3. Paste your Claude API key in the sidebar
...
SECURITY NOTE:
    Never hardcode API keys in source files. This version asks for the
    key at runtime via the sidebar and keeps it only in session state.
"""
...
st.sidebar.header("🔑 Claude API Key")
...
st.sidebar.caption("Get a key at https://console.anthropic.com/settings/keys")
...
if user_input:
    if not st.session_state.get("api_key"):
        st.warning("Please enter your Claude API key in the sidebar first.")
    else:
        ...
        try:
            ...
            # Send full conversation history (minus the system prompt) so the
            # model has context from earlier turns, not just the latest message.
            history = [
            ...
            reply = "⚠️ Invalid Claude API key. Please check the key in the sidebar."
